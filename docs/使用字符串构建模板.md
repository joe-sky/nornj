`NornJ`模板可以使用纯字符串或标签模板字符串(es6)构建：
* [使用纯字符串构建模板](#使用纯字符串构建模板)
  * [模板插值参数](#模板插值参数)
  * [过滤器](#过滤器)
  * [内置过滤器](#内置过滤器)
  * [块表达式](#块表达式)
  * [内置块表达式](#内置块表达式)
* [使用标签模板字符串(es6)构建模板](#使用标签模板字符串(es6)构建模板)
...
> 注：在使用`nornj-react`包后，插值参数语法默认改为使用单花括号如"{参数名}"，这样做是为了在`React`中使模板语法更加简练。在本项目的文档中，`React`相关的示例默认使用单花括号，其他则默认使用双花括号。
...
```
1. 元素节点名称作为替换参数时，不支持使用3个花括号形式的字符转义，但是会自动进行转义。在模板输出html字符串时，这样插值时则一定会执行转义；在输出vdom对象时，这样插值则不会进行转义，因为`React`会替我们进行转义。
2. 另外，利用此功能可以实现`React`的`高阶组件`，直接将组件类作为参数传入模板中即可。
...
* 在替换参数中可以使用过滤器，来对数据进行一些定制化操作。语法为"{替换参数 | 过滤器1 | 过滤器2...}"，如使用多个过滤器则会按顺序依次执行，如下所示：
```js
...
nj.registerFilter('test', function(obj) {
  ...
  console.log(nj.getData(this.data, 'id'));  //输出100
