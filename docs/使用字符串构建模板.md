...
params块可以提供另外一种定义元素节点参数的方式，模板标签的每个参数用params块内的p块包裹。用params块定义的节点参数可以内嵌或包裹其他表达式块，能够提供更丰富的动态生成逻辑。举例如下：
```js
const tmpl = nj`
<div name=foo>
  <#params>  //定义params块
    <#p {"name"}>bar</#p>  //节点参数键为字符串，值也为字符串(1)
    <#p {"id"}>{test}</#p>  //节点参数键为字符串，值为替换参数(2)
    <#p {"id2"}>  //节点参数值为表达式块的执行结果(3)
      <#each {list}>
        {no}
      </#each>
    </#p>
    <#each {list}>  //根据表达式块的执行结果动态构建节点参数(4)
      <#p {"data-id(" no ")"}>{"test" no}</#p>
    </#each>
  </#params>
...
1. 每个元素节点可以有任意多个params块子节点。params块内部可以定义任意个p块，每个p块为该元素节点定义一个节点参数。
2. p块开始标签内的参数为节点参数名称。使用字符串或替换参数都可以，也可以加过滤器。如例中(1)、(2)处所示。
3. p块的子节点为节点参数值。如果有多个子节点，则节点参数值为这些子节点值连成的一个完整字符串。如例中(3)处所示。
4. p块也可以用each块等其他表达式块包裹，这样就可以动态生成多个节点参数。如例中(4)处所示。
5. 使用p块定义节点参数的优先级会高于直接定义在节点的开始标签内，如例中(2)处的name属性就覆盖了节点标签中定义的name属性。
...
3. 还可以使用`nj.renderTmplExpr`方法来执行tmpl块内的模板，举例：
```js
class TestComp extends React.Component {
  render() {
    return nj.renderTmplExpr(
      this.props.tmpls[0],  //第一个参数为tmpl块内模板
      { text: 'test' }      //第二个参数为渲染参数
      /* 第三个参数可选，值为一个React组件或标签字符串(如div)，默认为div */
    );
  }
}
```
