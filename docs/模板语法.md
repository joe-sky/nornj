...
> 注：`NornJ`的计算属性都是只读的，并不能像`Vue`那样设置可写属性，因为在`NornJ`中也没有实现可写属性的意义。

```js
let html = nj`<div>{{#computed}}</div'>`({
...
<div>
  ...
  computed: (ctx) => {
    ...
    console.log(ctx.parent.data.list);  //输出[1]
...
| 名称           | 作用                      | 示例                                  |
|:---------------|:-------------------------|:--------------------------------------|
| prop           | 获取对象属性值             | {{value \| prop('foo')}}            |
| #              | 执行对象方法               | {{value \| #('substring', 0, 3)}}   |
| ==             | 等同于js中的"=="运算符     | {{value \| ==(foo)}}                     |
| ===            | 等同于js中的"==="运算符    | {{value \| ===(foo)}}                     |
| <              | 等同于js中的"<"运算符      | {{value \| <(foo)}}                      |
| <=             | 等同于js中的"<="运算符     | {{value \| <=(foo)}}                      |
| >              | 等同于js中的">"运算符      | {{value \| >(foo)}}                      |
...
| &&             | 等同于js中的"&&"运算符     | {{value \| &&(foo)}}                     |
| \|\|           | 等同于js中的"\|\|"运算符    | {{value \| \|\|(foo)}}                    |
| int            | 转换为int值                | {{value \| int(foo)}}                    |
| float          | 转换为float值              | {{value \| float(foo)}}                  |
| bool           | 转换为bool值               | {{value \| bool(foo)}}                   |

## 扩展标签

`NornJ`模板中可使用内置扩展标签来进行if、else、each等流程控制，也可以支持自定义扩展标签。
...
另外，如果prop标签没有被包裹其他扩展标签时，props也可以省略不写：
...
`NornJ`模板的插值变量内虽然不支持放入复杂js类型(如对象与数组)，但可以使用扩展标签来在模板中直接定义复杂类型。obj标签可以定义js对象，例如：
